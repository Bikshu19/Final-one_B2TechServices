...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch("https://final-one-b2techservices.onrender.com/api/auth/login", {
        method: "POST",
          ...
          <form onSubmit={handleLogin} className="px-8 py-8">
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-[#00809D] transition-colors"
                required
              />
            </div>

            <div className="mb-6">
              <label htmlFor="password" className="block text-[#0a1f44] text-sm font-semibold mb-2">
                Password
              </label>
              <input
                id="password"
